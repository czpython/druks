...
{% endif %}
- **Plan revision:** {{ build.plan_revision }}
- **Implementation revision:** {{ build.implementation_revision }}{% if build.implementation_revision == 0 %} (first attempt){% endif %}
{% if build.finalized_base_sha %}
- **base_sha:** `{{ build.finalized_base_sha }}`
{% endif %}
{% if build.finalized_pr_sha %}
- **head_sha:** `{{ build.finalized_pr_sha }}`
{% endif %}
...
{% for ac in build.acceptance_criteria %}
### {{ ac.id }}
...
{% endfor %}
{% endif %}
{% if build.reviewer_requirements %}
...
{% for req in build.reviewer_requirements %}
...
{% endfor %}
{% endif %}
{% if build.implementation_reviews %}
## Prior implementation review

{% for review in build.implementation_reviews %}
...
{% endfor %}
{% endif %}
{% if build.human_feedback %}
## Human feedback

{% for fb in build.human_feedback %}
