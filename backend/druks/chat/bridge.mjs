  ...
  async open(request) {
    ...
    try {
      ...
      const setup = {
        ...
        _meta: { claudeCode: { options: { disallowedTools: ["AskUserQuestion"] } } },
      };
...
const server = createServer(socket => {
  ...
  lines.once("line", line => {
    ...
    Promise.resolve().then(() => handle(JSON.parse(line)))
      ...
      .catch(error => {
        console.error(error);
        socket.end(JSON.stringify({ ok: false, error: "The bridge request failed. Read chat-bridge.log in the box." }) + "\n");
      });
  });
});
