...
from druks.sandbox.layout import get_chat_conversation_root
...
from .constants import CHAT_AUDIENCE
from .enums import MessageRole, MessageState
from .exceptions import ChatBoxGone, ChatBridgeError, ChatHarnessError
...
async def get_box(
    ...
    """The account's box for a new turn: a live box that holds the Chat agent's
    current secrets, or a new one."""
    server = get_druks_mcp_server(allowed_tools=())
    token = await get_druks_account_token(
        session, account_id, server.allowed_tools, audience=CHAT_AUDIENCE
    )
    refs = [
        *config.secret_refs,
        SecretRef(
    ...
    async with chat_lock(f"chat:account:{account_id}"):
        ...
        if identity:
            with suppress(ChatBoxGone):
                return await attach_box(session, identity), identity
        # One box per account: a box with other secrets gives way to the new one.
...
async def get_running_box(session: AsyncSession, account_id: str) -> Host:
    """The account's live box, whatever its secrets: the box a sent turn runs in."""
    identity = await SandboxIdentity.lookup(
        session, account_id=account_id, run_id=None, scoped_to="chat"
    )
    if identity:
        return await attach_box(session, identity)
    raise ChatBoxGone("The account has no live Chat box.")


async def attach_box(session: AsyncSession, identity: SandboxIdentity) -> Host:
    await session.commit()
    try:
        ...
        raise ChatBoxGone("The account's Chat box is gone.") from error
    ...
    async with chat_lock(f"chat:{conversation.id}:delivery"):
        while message := await conversation.get_unanswered_message(session):
            if message.state == MessageState.PENDING:
                ...
                host, identity = await get_box(session, conversation.account_id, config)
                try:
                    bridge = Bridge(host)
                ...
                continue
            try:
                host = await get_running_box(session, conversation.account_id)
            except ChatBoxGone:
...
async def send_turn(
    ...
    if not status["sessionId"] and conversation.session_file:
        root = get_chat_conversation_root(host.ssh_username, conversation.id)
