...
from druks.sandbox.layout import get_chat_bridge_path, get_work_root
...
class Bridge:
    ...
    async def start(self) -> None:
        """Start the bridge in the box unless it already answers."""
        if await self.is_running():
            return
        script = shlex.quote(get_chat_bridge_path(self.host.ssh_username))
        root = shlex.quote(get_work_root(self.host.ssh_username))
        result = await self.host.exec(
            [
                "sh",
                "-c",
                f"mkdir -p {root} && nohup setsid node {script} {CHAT_BRIDGE_PORT} "
        ...
        try:
            ...
            for event, notification in zip(events, notifications, strict=True):
                ...
                if update["sessionUpdate"] == "tool_call":
                    # Claude's adapter names the tool only in its own metadata.
                    claude = update.get("_meta", {}).get("claudeCode", {})
                    update["name"] = claude.get("toolName", update["title"])
        return events
