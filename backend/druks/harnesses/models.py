...
class ProviderSubscription(Base, Uuid7Pk):
    ...
    async def get(cls, subscription_id: str) -> "ProviderSubscription | None":
        return await db_session().scalar(
            select(cls).where(cls.id == subscription_id, cls.disconnected_at.is_(None))
        )
    ...
    async def get_for_account(
        cls, provider_id: str, account_id: str | None
    ) -> "ProviderSubscription | None":
        return await db_session().scalar(
            select(cls).where(
                cls.provider == provider_id,
                cls.account_id == account_id,
                cls.disconnected_at.is_(None),
            )
        )
    ...
    async def list_all(cls) -> list["ProviderSubscription"]:
        return list(
            await db_session().scalars(
                select(cls).where(cls.disconnected_at.is_(None)).order_by(cls.provider, cls.id)
            )
        )
    ...
    async def list_for_account(cls, account_id: str) -> list["ProviderSubscription"]:
        stmt = (
            select(cls)
            .where(cls.account_id == account_id, cls.disconnected_at.is_(None))
            .order_by(cls.provider)
        )
    ...
    async def list_for_provider(cls, provider_id: str) -> list["ProviderSubscription"]:
        stmt = (
            select(cls)
            .where(cls.provider == provider_id, cls.disconnected_at.is_(None))
            .order_by(cls.id)
        )
        return list(await db_session().scalars(stmt))
    ...
    async def reload(cls, subscription_id: str) -> "ProviderSubscription | None":
        ...
        return await db_session().scalar(
            select(cls)
            .where(cls.id == subscription_id, cls.disconnected_at.is_(None))
    ...
    async def connect(
        ...
        *,
        ...
        row = await session.scalar(
            select(cls).where(cls.provider == provider, cls.account_id == account.id)
        )
        if not row:
    ...
    def is_connected(self) -> bool:
        return not self.disconnected_at and (
            not self.expires_at or self.expires_at > datetime.now(UTC)
        )

    async def update_payload(self, payload: dict, *, expires_at: datetime | None) -> None:
        await db_session().execute(
            update(type(self))
            .where(type(self).id == self.id, type(self).disconnected_at.is_(None))
            .values(payload=payload, expires_at=expires_at)
        )
        await db_session().refresh(self)

    async def delete(self) -> None:
        self.disconnected_at = Base.utc_now()
        self.payload = {}
...
class ProviderKey(Base):
    ...
    async def get(cls, provider_id: str) -> "ProviderKey | None":
        return await db_session().scalar(
            select(cls).where(cls.provider == provider_id, cls.disconnected_at.is_(None))
        )
    ...
    async def list_all(cls) -> list["ProviderKey"]:
        return list(
            await db_session().scalars(
                select(cls).where(cls.disconnected_at.is_(None)).order_by(cls.provider)
            )
        )
