...
async def list_projects() -> ProjectsResponse:
    rows = list(await db_session().scalars(select(Project).order_by(Project.name)))
...
async def update_project_repo(
    ...
    row = await ProjectRepo.get_in_project(project_id=project_id, repo_id=repo_id)
    if not row:
...
async def list_work_items_history(
    ...
    items = [DashboardItem.model_validate(wi) for wi in await WorkItem.list_handoff(limit=clamped)]
...
async def start_work_item(
    ...
    tracker = await Ship.get_tracker()
    if not tracker:
