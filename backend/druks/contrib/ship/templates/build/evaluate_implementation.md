...
- For a command labeled CI-primary, CI on exactly `head_sha` is authoritative. Do not execute the profile command locally. First run `gh pr view <pr_number> --json headRefOid` and verify
  ...
  `gh run list --commit <head_sha>` (or an equivalently commit-pinned GitHub query), and inspect
...
- For a command labeled local execution, run that exact configured command when feasible and
  report its result. This includes legacy profiles and operator-pinned commands with no CI
  provenance. The environmental-blocker rules below continue to apply.
...
1. **Environmental** — a local-only verification command cannot run in this sandbox because
   the runtime or toolchain is wrong/missing, not because the code is wrong. Examples: "the
   production build needs Node >=20.9.0 but the sandbox has 18.x", "the typecheck binary exits
   printing its help instead of running", "the test interpreter/deps aren't installed".
   Report the exact command and blocker, mark the check `not_run`, and return `blocked`. Do not
   fail the implementer for a check the box physically cannot execute, and do not infer CI
   credit for a command with no mapped check.
...
GITHUB CHECKS — consult profiler-attributed checks as the primary verification evidence using
the head-SHA-pinned procedure above (`gh` is authenticated):
- A red mapped check this diff caused is a high finding and a fail verdict. Inspect its run/log,
  and use default-branch history for the same check to assess whether it is pre-existing.
- Do not infer CI coverage for an unmapped command from a similarly named workflow or check.
- A local-only command blocked by unavailable dependencies, private indexes, credentials, or
  toolchains follows the environmental-blocker path; unrelated green checks do not grant it CI
  credit.
