...
3. **Pre-existing baseline failure** — the failing check also fails on `pr_base_sha` / in code the diff did not touch (confirm before claiming it). The diff didn't introduce it, so it isn't this PR's regression. Mark it `not_run`/baseline; if it is the ONLY blocker, return `blocked` with that note rather than `fail`.
...
- Where a green check already ran a configured verification command, record that command as passing, name the check, and do not run it yourself. Rerunning what CI just ran spends a full suite for no new information.
- A failing check this diff caused is a high finding and a fail verdict. Inspect its log and reproduce only the single failing target it names, never the whole suite. Confirm it also fails on `pr_base_sha` before claiming it is a pre-existing baseline failure (shape 3 of the INFEASIBLE-BLOCKER GATE).
...
- If a command you must run yourself cannot run because repo dependencies, private indexes, or credentials are unavailable, report that check as not_run. Green GitHub checks covering the same ground make that gap non-blocking: return pass, not blocked, and name in `body` which GitHub checks stood in.
