...
async def build_feed(
    *,
    ...
    items = [FeedItem.model_validate(event) for event in await _events(app, before)]
...
async def _events(app: str | None, before: int | None) -> list[Event]:
    ...
    return list((await db_session().scalars(stmt.limit(_FETCH_LIMIT))).all())
