...
from druks.mcp.exceptions import MissingEndpointError, ReservedServerNameError
...
def test_appliance_mcp_url_refuses_an_unset_endpoint(monkeypatch):
    ...
    with pytest.raises(MissingEndpointError, match="urls.endpoint"):
...
async def test_a_workspace_that_operates_the_appliance_injects_its_mcp(druks_db, monkeypatch):
    ...
    kwargs = await workspace.with_mcp_servers(druks_db, account.id, call_id="call-1")

    server = next(s for s in kwargs["mcp_servers"] if s.name == DRUKS_SERVER_NAME)
    assert server.url == "http://host.docker.internal:8001/mcp"
    found = await OperatorToken.lookup(
        kwargs["extra_env"][get_bearer_token_env_var(DRUKS_SERVER_NAME)]
    )
...
async def test_a_plain_workspace_gets_no_inward_credential(druks_db, monkeypatch):
    ...
    kwargs = await Workspace(host=_ApplianceHost()).with_mcp_servers(  # type: ignore[arg-type]
        druks_db, account.id, call_id="call-1"
    )

    assert DRUKS_SERVER_NAME not in {server.name for server in kwargs.get("mcp_servers") or ()}
...
async def test_a_registry_server_cannot_claim_the_appliance_name(druks_db, monkeypatch):
    ...
    monkeypatch.setattr(
        Workspace,
        "get_mcp_delivery",
        AsyncMock(return_value=((McpServer(name=DRUKS_SERVER_NAME, url="http://elsewhere"),), [])),
    )
    workspace = Workspace(
    ...
    with pytest.raises(ReservedServerNameError):
        await workspace.with_mcp_servers(druks_db, account.id, call_id="call-1")
...
async def test_the_credential_is_gone_after_the_call(druks_db, monkeypatch):
    ...
    token = host.run_agent.await_args.kwargs["extra_env"][
        get_bearer_token_env_var(DRUKS_SERVER_NAME)
    ]
