...
async def test_onboarding_after_the_only_subscription_is_revoked(
    ...
    await subscription.revoke(db_session(), reason)
...
async def test_none_zero_setup_flow_creates_the_operator(tmp_path, monkeypatch, druks_db):
    ...
    assert await VaultSecret.lookup(
        druks_db, SecretKind.SUBSCRIPTION, Audience.provider("anthropic"), account.id
    )
...
async def test_a_pasted_key_is_the_providers_and_names_its_paster(tmp_path, druks_db):
    ...
    stored = await VaultSecret.lookup(druks_db, SecretKind.STATIC, Audience.provider("anthropic"))
...
async def test_a_second_pasted_key_replaces_the_first(tmp_path, druks_db):
    ...
    [stored] = await VaultSecret.list_keys(druks_db)
...
async def test_reconnect_records_provider_email_but_keeps_the_operator(
    ...
    codex = await VaultSecret.lookup(
        druks_db, SecretKind.SUBSCRIPTION, Audience.provider("openai"), account.id
    )
