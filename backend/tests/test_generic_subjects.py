...
class Thing(StoredSubject):
    ...
    def get_summary(self) -> _ThingSummary:
        return _ThingSummary(id=self.id, key=self.key, title=TITLES[self.id])
...
class Ticket(Subject):
    ...
    def get_summary(self) -> _ThingSummary:
        return _ThingSummary(id=self.id, key=self.key, title=self.id.rpartition("#")[2])
...
def test_a_subject_id_is_a_string_whatever_the_row_is_keyed_by(druks_db):
    ...
    with pytest.raises(ValidationError):
        _ThingSummary(id=7, key="7", title=7)


def test_a_summary_carries_the_subjects_own_key(druks_db):
    assert SubjectSummary.model_validate(Thing(id=7)).key == "thing 7"
    assert SubjectSummary.model_validate(Ticket(id="owner/repo#7")).key == "owner/repo#7"
    ...
    with pytest.raises(ValidationError):
        _ThingSummary(id=1, key="   ", title="First")
...
async def test_status_aggregates_across_runs_and_timeline_spans_them(client: TestClient, druks_db):
    ...
    assert detail["summary"] == {"id": "1", "key": "thing 1", "title": "First"}
...
async def test_an_id_spanning_separators_reaches_the_board_and_its_page(
    ...
    assert detail["summary"] == {"id": "owner/repo#7", "key": "owner/repo#7", "title": "7"}
    assert detail["status"]["state"] == "parked"
