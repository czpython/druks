...
async def test_get_project_returns_the_summary_or_404(client: TestClient):
    created = (await client.post("/api/ship/projects", json={"name": "Acme"})).json()

    fetched = await client.get(f"/api/ship/projects/{created['id']}")
    assert fetched.status_code == 200
    assert fetched.json() == created
    assert (await client.get("/api/ship/projects/999999")).status_code == 404
...
async def test_adding_a_repo_dispatches_a_profile_run(client: TestClient, monkeypatch):
    ...
    project = (await client.post("/api/ship/projects", json={"name": "Acme"})).json()
    repo = (
        await client.post(
...
async def test_profile_endpoint_dispatches(client: TestClient, monkeypatch):
    ...
    response = await client.post(f"/api/ship/projects/{project.id}/repos/{repo.id}/profile")
...
async def test_deleting_a_project_cascades_its_work_items_and_spares_others(
    ...
    response = await client.delete(f"/api/ship/projects/{target_id}")
...
async def test_the_repo_subject_read_side_mounts(client: TestClient, druks_db):
    ...
    (row,) = (await client.get("/api/ship/project_repo")).json()["rows"]
    assert row["summary"]["id"] == str(repo.id)
    assert row["summary"]["fullName"] == "acme/widget"
    assert row["status"]["state"] == "running"

    detail = (await client.get(f"/api/ship/project_repo/{repo.id}")).json()
