...
async def test_every_subject_shows_itself(druks_db):
...
async def test_feed_paginates_same_second_events_without_loss_or_repeat(druks_db):
    ...
    for _ in range(10):  # bounded so a paging bug can't loop forever
        page, cursor = await build_feed(before=int(cursor) if cursor else None, limit=2)
