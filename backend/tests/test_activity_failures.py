...
def test_codex_spend_cap_preserves_the_terminal_message(stream):
    ...
    with pytest.raises(HarnessSpendCapError) as raised:
        CodexHarness.check_returncode(result)

    assert raised.value.code == "spend_cap"
...
async def test_spend_cap_records_one_terminal_attempt_without_quota_wait(
    ...
    async def run_agent(self, **context):
        nonlocal attempts
        attempts += 1
    ...
    try:
        with pytest.raises(HarnessSpendCapError):
...
@pytest.mark.parametrize(
    ("error", "retry", "delays"),
    [
        (HarnessError, Retry.NEVER, ()),
        (HarnessTimeoutError, Retry.NEVER, ()),
        (HarnessAuthError, Retry.NEVER, ()),
        (HarnessRateLimitError, Retry.QUOTA, ()),
        (HarnessUsageLimitError, Retry.QUOTA, ()),
        (HarnessOverloadedError, Retry.TRANSIENT, (300, 900)),
    ],
)
def test_existing_error_retry_policies_remain_intact(error, retry, delays):
    assert error.retry is retry
    assert error.retry_delays == delays
