...
@pytest.mark.parametrize(("document", "scopes"), [("oauth", "openid email"), ("resource", [1])])
...
async def test_root_openid_metadata_cannot_change_the_authority(provider, field):
    ...
    async with oauth._http() as client:
        authorization_server, _ = await oauth._discover(client, NAME, provider["server"])
        identity_provider = await oauth._resolve_identity_provider(
            client, NAME, authorization_server
        )
    assert authorization_server == provider["oauth"]
    assert identity_provider == provider["oauth"]
