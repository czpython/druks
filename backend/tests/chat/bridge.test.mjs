...
test("The bridge streams detached turns, isolates archives, cancels, and reloads sessions", async t => {
  ...
  const home = path.join(temporary, "box");
  await fs.mkdir(home);
  ...
  await fs.writeFile(adapter, [
    ...
    " if (p._meta.claudeCode.options.disallowedTools[0] !== 'AskUserQuestion') throw Error('asks');",
  ...
  const start = conversation => ({
    ...
    sessionFiles: ".claude/projects", mode: "bypassPermissions", model: "claude-opus-4-7",
  ...
  const restoredHome = path.join(temporary, "restored-box");
