...
from druks.chat.constants import CHAT_AUDIENCE
from druks.chat.enums import ConversationSource, MessageRole, MessageState
from druks.chat.exceptions import ChatBoxGone, ChatBridgeError
...
@pytest.fixture
async def box(druks_db, conversation, monkeypatch):
    ...
    monkeypatch.setattr(service, "get_box", AsyncMock(return_value=(host, identity)))
    monkeypatch.setattr(service, "get_running_box", AsyncMock(return_value=host))
    monkeypatch.setattr(
        service,
        "get_default_config",
        AsyncMock(
...
async def test_chat_key_is_separate_from_the_build_key(druks_db, conversation):
    build = await get_druks_account_token(
        ...
        audience=Audience.mcp("druks"),
    )
    chat = await get_druks_account_token(
        druks_db,
        conversation.account_id,
        (),
    ...
    assert (
        await get_druks_account_token(druks_db, conversation.account_id, (), audience=CHAT_AUDIENCE)
    ).id == chat.id
...
async def test_chat_identity_authenticates_until_lease_expiry_and_is_not_an_orphan(
    ...
    token = await get_druks_account_token(
        druks_db, conversation.account_id, (), audience=CHAT_AUDIENCE
    )
...
async def test_conversations_share_the_account_box_until_its_secrets_change(
    druks_db, conversation, monkeypatch
):
    hosts = {}

    async def provision(**values):
        host = SimpleNamespace(id=f"box-{len(hosts) + 1}", aclose=AsyncMock())
        hosts[host.id] = host
    ...
    first_host, _identity = await service.get_box(druks_db, conversation.account_id, config)
    second_host, _identity = await service.get_box(druks_db, second.account_id, config)
    login = await get_druks_account_token(
        druks_db, conversation.account_id, (), audience=Audience.mcp("login")
    )
    moved = SimpleNamespace(
        secret_refs=[SecretRef(name="claude_token", secret_id=login.id)], secrets={}
    )
    moved_host, _identity = await service.get_box(druks_db, second.account_id, moved)
...
async def test_delivered_turn_is_never_sent_again_after_a_transport_failure(
    druks_db, conversation, box, monkeypatch
):
...
async def test_missing_box_interrupts_delivered_turn_without_provisioning(
    druks_db, conversation, monkeypatch
):
    [message] = await list_messages(druks_db, conversation)
    message.state = MessageState.DELIVERED
    await druks_db.commit()
    get_running_box = AsyncMock(side_effect=ChatBoxGone("expired"))
    monkeypatch.setattr(service, "get_running_box", get_running_box)

    await service.deliver_pending(druks_db, conversation)

    assert message.state == "interrupted"
    get_running_box.assert_awaited_once_with(druks_db, conversation.account_id)
...
async def test_recovery_reads_live_events_then_saves_reply_and_replaces_archive(
    druks_db, conversation, box, tmp_path, monkeypatch, terminal_state
):
    ...
    updates = [
        ...
        {"sessionUpdate": "agent_message_chunk", "content": {"type": "text", "text": "Found it."}},
        {
            ...
            "title": "Get ticket",
            "status": "in_progress",
            "name": "mcp__druks__get_gate",
            "_meta": {"claudeCode": {"toolName": "mcp__druks__get_gate"}},
...
async def test_new_box_restores_archive_and_drains_pending_messages(
    druks_db, conversation, box, tmp_path, monkeypatch
):
    monkeypatch.setenv("DRUKS_DATA_DIR", str(tmp_path))
    previous = FileRecord(
        id="saved-session",
...
async def test_stop_during_startup_keeps_the_box_and_sends_only_the_next_message(
    druks_db, conversation, box, monkeypatch
):
    host, identity = box
    first = await conversation.get_unanswered_message(druks_db)
    ...
    async def get_box(session, account_id, config):
        nonlocal box_requests
        box_requests += 1
        await session.commit()
        startup.set()
    ...
    monkeypatch.setattr(service, "get_box", get_box)
    monkeypatch.setattr(Bridge, "request", request)
...
async def test_cancel_route_only_changes_the_requested_pending_message(
    ...
    get_running_box = AsyncMock(
        side_effect=AssertionError("Pending cancellation must not reach the box.")
    )
    monkeypatch.setattr(service, "get_running_box", get_running_box)
    headers = {"X-User": "owner@example.com"}
    ...
    assert first.state == "pending"
    get_running_box.assert_not_called()
