...
import druks.contrib.software_factory.subscribers  # noqa: F401 — registers the pr.closed subscriber
import pytest
...
def _stub_config_fetch(monkeypatch):
    # The external-close path resolves the repo's live .druks/software_factory/config.yml;
...
async def test_external_close_honors_delete_branch_policy(druks_db, tmp_path, monkeypatch):
    """delete_branch: false in the repo's live .druks/software_factory/config.yml keeps the
    head branch on an external close."""
    from druks.contrib.software_factory import models as build_models
