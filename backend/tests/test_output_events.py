    ...
    for call in calls:
        for _ in range(2):
            await Artifact.record(
                ...
                event={"topic": "review.completed", "summary": "No unresolved findings."},
            )
...
async def test_artifact_without_an_event_records_none(druks_db, output_calls, tmp_path):
    db_session.registry.set(druks_db)
...
async def test_rollback_removes_the_artifact_and_its_event(druks_db, output_calls, tmp_path):
    db_session.registry.set(druks_db)
    _, _, calls = output_calls
    call_id = calls[0].id
