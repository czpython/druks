...
async def test_manifest_records_the_delivered_capability_set(druks_db):
    ...
    await models.McpServer.create(
        druks_db, name="linear", url=_LINEAR_URL, secret_headers={"Authorization": _TOKEN}
    )
...
async def test_manifest_stays_presence_only_for_a_declared_header_server(druks_db):
    ...
    secret header counts as the credential present."""
    await models.McpServer.create(
        druks_db,
        name="grafana",
    ...
    assert grafana_entry["token_present"] is True
