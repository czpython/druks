...
go next. Box startup continues. The next message reuses the box, or the box
...
real tool name, and status. Select the row to read its input and output.
...
Each account has one leased box that serves all its conversations. When the
Chat credentials change, the next turn replaces the box, and a turn
still running in the old box ends as `interrupted`. A detached
bridge runs in the box and starts one ACP adapter per conversation. Druks
...
The agent keeps its process in the box. Each delivery is a DBOS workflow.
...
### The bridge or box dies mid-turn
...
Nothing renews an idle box. It expires with its lease. The next message gets
a new box and reloads the saved session files. Each turn renews the box lease
and its identity expiry. The lease is 150 minutes.
