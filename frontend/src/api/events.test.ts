...
it('sends each filter value literally', async () => {
  ...
  await api.listEvents({ q: 'ACME%_ &?', app: 'field_notes', kind: 'summary.ready',
    from: '2026-09-09T00:00:00Z', until: '2026-09-10T00:00:00Z', before: '42' })
  expect(fetcher).toHaveBeenCalledWith(
    '/api/events?q=ACME%25_+%26%3F&app=field_notes&kind=summary.ready'
      + '&from=2026-09-09T00%3A00%3A00Z&until=2026-09-10T00%3A00%3A00Z&before=42',
    expect.anything(),
  )
  await api.listEventKinds('field_notes')
  expect(fetcher).toHaveBeenLastCalledWith('/api/events/kinds?app=field_notes', expect.anything())
