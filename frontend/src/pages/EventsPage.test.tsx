...
it('reads a selection outside the loaded pages and leaves no gap when it closes', async () => {
  ...
  history.mockImplementation(async (params) => params?.limit === 1
    ? { items: [{ ...result, id: 'event:5', seq: 5, subjectLabel: 'Pump E' }], cursor: '10:20:10', nextCursor: null }
    : { items: [result], cursor: '10:20:10', nextCursor: '10' })
  mount()
...
it('retries the exact saved artifact and replaces it on another selection', async () => {
  history.mockResolvedValue({ items: [result, { ...result, id: 'event:9', seq: 9, artifactId: 'saved-nine', subjectLabel: 'Pump B' }], cursor: '10:20:10', nextCursor: null })
