...
const result: FeedItem = {
  ...
  subjectType: 'note', subjectId: '7', subjectKey: 'Pump A',
...
it.each(['field_notes', 'software_factory'])('selects the owning app for %s topics in history and the stream', async (owner) => {
  registerAppUI({ name: 'field_notes', routes: [], activity: ({ topic }) => ({ label: topic === 'merged' ? 'Notes combined' : undefined }) })
...
it('buffers and deduplicates arrivals while reading, then resumes from the last complete snapshot', async () => {
  ...
  act(() => {
    source().emit('open')
    source().emit('message', { ...result, id: 'event:12', seq: 12, subjectKey: 'New work' })
    source().emit('message', { ...result, id: 'event:12', seq: 12, subjectKey: 'New work' })
  ...
  expect(new URL(source().url, window.location.origin).searchParams.get('cursor')).toBe('10:30:10')
  act(() => {
    source().emit('message', { ...result, id: 'event:9', seq: 9, subjectKey: 'Late work' })
    source().emit('message', { ...result, id: 'event:9', seq: 9, subjectKey: 'Late work' })
  })
...
it('starts live updates on an empty feed from its read snapshot', async () => {
  history.mockResolvedValue({ items: [], cursor: '10:20:10', nextCursor: null })
  mount()
...
it('reads a selection outside the loaded pages and leaves no gap when it closes', async () => {
  ...
  history.mockImplementation(async (params) => params?.limit === 1
    ? { items: [{ ...result, id: 'event:5', seq: 5, subjectKey: 'Pump E' }], cursor: '10:20:10', nextCursor: null }
    : { items: [result], cursor: '10:20:10', nextCursor: '10' })
  mount()
...
it('shows Factory review findings through the shared saved-result renderer', async () => {
  history.mockResolvedValue({ items: [{ ...result, app: 'software_factory', topic: 'review.completed',
    subjectType: 'work_item', subjectId: '42', subjectKey: 'DRU-42', payload: { ...result.payload, artifact_id: 'review-ten' },
  }], cursor: '10:20:10', nextCursor: null })
