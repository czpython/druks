...
describe('Chat page', () => {
  ...
  it('shows connection progress at the next reply without infrastructure copy', async () => {
    ...
    expect(screen.queryByText(/get the box|start the agent|No messages yet/i)).toBeNull()
    expect((screen.getByRole('textbox') as HTMLTextAreaElement).disabled).toBe(false)
    fireEvent.click(screen.getByRole('button', { name: 'Stop' }))
    await waitFor(() => expect(chatApi.stop).toHaveBeenCalledExactlyOnceWith(conversation.id, message.id))
  })
  ...
  it('streams plain agent text and expandable tool input and output without row times', async () => {
    mount()
    const socket = await connected()
    const emit = (sequence: number, update: Extract<ConversationAction, { type: 'event' }>['notification']['update']) => socket.receive({ type: 'event', messageId: '10', epoch: 'box', sequence, notification: { sessionId: 'agent', update } })
    emit(1, { sessionUpdate: 'agent_message_chunk', content: { type: 'text', text: 'I will check the gates.' } })
    emit(2, { sessionUpdate: 'tool_call', toolCallId: 'gate', title: 'Read the approval gate', name: 'mcp__druks__get_gate', rawInput: { run: 'example' }, status: 'in_progress' })
    ...
    expect(within(tool).getByText('get_gate')).toBeTruthy()
  ...
  it('reconnects the stream without resending a saved message or duplicating text', async () => {
    ...
    const chunk: ConversationAction = { type: 'event', messageId: '10', epoch: 'box', sequence: 1, notification: { sessionId: 'agent', update: { sessionUpdate: 'agent_message_chunk', content: { type: 'text', text: 'One reply.' } } } }
    first.receive(chunk)
    vi.useFakeTimers()
