...
export function AppHomePage({ app, description, subjectTypes }: Props) {
  return (
    <Page
      scroll="internal"
    ...
    >
      {subjectTypes.length === 0 ? (
        ...
        subjectTypes.map((subjectType) => (
          <SubjectBoard key={subjectType} app={app} subjectType={subjectType} />
        ))
      )}
    </Page>
  )
...
function SubjectBoard({ app, subjectType }: { app: string; subjectType: string }) {
  const [rows, setRows] = useState<SubjectRow[] | null>(null)
  const [, navigate] = useLocation()

  useSSE(subjectApi.boardStream(app, subjectType), {
    handlers: useMemo(
