...
describe('Chat events', () => {
  it('keeps text and tool calls in order and merges partial tool updates', () => {
    ...
    state = conversationReducer(state, event(2, { sessionUpdate: 'tool_call', toolCallId: 'gate', title: 'Read the gate', name: 'mcp__druks__get_gate', rawInput: { run: 'run-one' }, status: 'pending' }))
    ...
    expect(state.turns['2']?.rows).toEqual([
      ...
      { type: 'tool', call: expect.objectContaining({ title: 'Read the gate', name: 'mcp__druks__get_gate', rawInput: { run: 'run-one' }, status: 'completed', rawOutput: { gate: 'approval' } }) },
  ...
  it('does not duplicate replayed chunks and resets the cursor for a new box', () => {
