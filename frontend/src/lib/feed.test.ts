...
it('uses another app formatter for the same topic', () => {
  registerAppUI({ name: 'publishing', routes: [],
    activityLabel: () => 'Notes combined',
    activity: ({ payload }) => ({ context: String(payload.summary), icon: FileCheck2, tone: 'positive' }),
  })
  const line = eventLine(event({ app: 'publishing', topic: 'merged', subjectLabel: 'NOTE-7',
    payload: { title: 'Recorded note title', summary: 'Three notes combined.' } }))
  expect(line).toMatchObject({ subject: 'NOTE-7', title: 'Recorded note title', label: 'Notes combined',
...
it('explains a shared spend-cap failure from its recorded code', () => {
  const line = eventLine(event({ app: 'field_notes', topic: 'workflow.failed',
    payload: { failure_code: 'spend_cap', failure: 'codex exited with 1. Original text.' } }))
  expect(line.context).toBe('Workspace spend cap reached')
  expect(line.guidance).toBe('Ask a workspace owner to increase the spend cap before continuing.')
