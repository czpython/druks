  ...
  routes: [
    {
      ...
      render: () => (
        <>
          <h1>Notes home</h1>
          <Link href="/notes/one">Open note</Link>
        </>
      ),
    },
...
describe('command center navigation', () => {
  it('keeps app destinations stable on list and detail pages', async () => {
    ...
    expect(
      within(screen.getByRole('complementary', { name: 'Druks navigation' })).getByText(
        account.username,
      ),
    ).toBeTruthy()
  })
  ...
  it('closes phone navigation before it opens the settings route', async () => {
    renderApp()
  ...
  it('waits for the roster before it rejects an installed app deep link', async () => {
    ...
    vi.mocked(api.listApps).mockReturnValueOnce(
      new Promise((resolve) => {
  ...
  it('keeps a large roster searchable with complete long names', async () => {
    const apps = Array.from({ length: 40 }, (_, index) => ({
      ...roster[2]!,
      name: `department_${index}_with_a_long_app_name`,
    }))
    ...
    expect(
      within(screen.getByRole('navigation', { name: 'Apps' })).getAllByRole('link'),
    ).toHaveLength(1)
